...
// Kata kunci pencarian cepat ditambahkan di sini
const TRAPS = ["X", "Y", "W", "IF", "AH", "EH", "OX" ,"EX", "OH", "AX", "CY", "AO", "LY", "HY", "GY"];
...
function updateList() {
    ...
    if (query.length > 0) {
        ...
        expandablePanels.classList.remove('hidden');
    }

    let display = [];
    if (query.length > 0) {
        display = wordDatabase.filter(w => w.startsWith(query) && !wrongWordsSet.has(w));
        display.sort((a,b) => (TRAPS.some(e=>a.endsWith(e)) ? 0 : 1) - (TRAPS.some(e=>b.endsWith(e)) ? 0 : 1) || a.length - b.length);
        ...
        shuf(eh); shuf(xy); shuf(others);
        let max = Math.max(eh.length, xy.length, others.length);
        for(let i=0; i<max; i++) {
    ...
    display.slice(0, maxDisplayLimit).forEach(word => {
        ...
        let cls = "normal";
        if (word.endsWith("EH")) cls = "trap-eh";
